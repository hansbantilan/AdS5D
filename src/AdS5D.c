...
//
// application interface functions for AdS5D
//=============================================================================
...
// complex scalar
real phi4_qb_amp_1,phi4_qb_x0_1[2],phi4_width_1[2],phi4_qb_w0_1,phi4_qb_r0_1,phi4_qb_delta_1;
real phi4_qb_amp_2,phi4_qb_x0_2[2],phi4_width_2[2],phi4_qb_w0_2,phi4_qb_r0_2,phi4_qb_delta_2;
...
real *phi4_r,*phi4_r_n,*phi4_r_np1,*phi4_r_nm1;
real *phi4_i,*phi4_i_n,*phi4_i_np1,*phi4_i_nm1;
...
real *phi4_r_t,*phi4_r_t_n;
real *phi4_i_t,*phi4_i_t_n;
...
int phi4_r_gfn,phi4_r_n_gfn,phi4_r_np1_gfn,phi4_r_nm1_gfn;
int phi4_i_gfn,phi4_i_n_gfn,phi4_i_np1_gfn,phi4_i_nm1_gfn;
...
int phi4_r_t_gfn,phi4_r_t_n_gfn;
int phi4_i_t_gfn,phi4_i_t_n_gfn;
...
int phi1_res_gfn,phi4_res_gfn,gb_res_gfn;
...
void set_gfns(void)
{
    ...
    if ((phi4_r_gfn     = PAMR_get_gfn("phi4_r",PAMR_MGH, 0))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_r_nm1_gfn = PAMR_get_gfn("phi4_r",PAMR_AMRH,3))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_r_n_gfn   = PAMR_get_gfn("phi4_r",PAMR_AMRH,2))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_r_np1_gfn = PAMR_get_gfn("phi4_r",PAMR_AMRH,1))<0) AMRD_stop("set_gnfs error",0);

    if ((phi4_i_gfn     = PAMR_get_gfn("phi4_i",PAMR_MGH, 0))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_i_nm1_gfn = PAMR_get_gfn("phi4_i",PAMR_AMRH,3))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_i_n_gfn   = PAMR_get_gfn("phi4_i",PAMR_AMRH,2))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_i_np1_gfn = PAMR_get_gfn("phi4_i",PAMR_AMRH,1))<0) AMRD_stop("set_gnfs error",0);
    ...
    if ((phi4_r_t_gfn = PAMR_get_gfn("phi4_r_t",PAMR_MGH,0))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_r_t_n_gfn = PAMR_get_gfn("phi4_r_t",PAMR_AMRH,2))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_i_t_gfn = PAMR_get_gfn("phi4_i_t",PAMR_MGH,0))<0) AMRD_stop("set_gnfs error",0);
    if ((phi4_i_t_n_gfn = PAMR_get_gfn("phi4_i_t",PAMR_AMRH,2))<0) AMRD_stop("set_gnfs error",0);
    ...
    if ((phi4_res_gfn  = PAMR_get_gfn("phi4_res",PAMR_AMRH,1))<0) AMRD_stop("set_gnfs error",0);
...
void ldptr(void)
{
   ...
   phi4_r     = gfs[phi4_r_gfn-1];
   phi4_r_n   = gfs[phi4_r_n_gfn-1];
   phi4_r_np1 = gfs[phi4_r_np1_gfn-1];
   phi4_r_nm1 = gfs[phi4_r_nm1_gfn-1];

   phi4_i     = gfs[phi4_i_gfn-1];
   phi4_i_n   = gfs[phi4_i_n_gfn-1];
   phi4_i_np1 = gfs[phi4_i_np1_gfn-1];
   phi4_i_nm1 = gfs[phi4_i_nm1_gfn-1];
   ...
   phi4_r_t = gfs[phi4_r_t_gfn-1];
   phi4_r_t_n = gfs[phi4_r_t_n_gfn-1];
   phi4_i_t = gfs[phi4_i_t_gfn-1];
   phi4_i_t_n = gfs[phi4_i_t_n_gfn-1];
...
//=============================================================================
void AdS5D_var_pre_init(char *pfile)
{
   AMRD_echo_params=1;
...
void AdS5D_var_post_init(char *pfile)
{
   int i,j,ind;
   char buf[64];
   real rmin,deltar;
   ...
   {
      printf("===================================================================\n");
      printf("Reading AdS5D parameters:\n\n");
   }
   ...
   phi4_qb_amp_1=phi4_qb_x0_1[0]=phi4_qb_x0_1[1]=phi4_qb_w0_1=phi4_qb_r0_1=0;
   phi4_qb_amp_2=phi4_qb_x0_2[0]=phi4_qb_x0_2[1]=phi4_qb_w0_2=phi4_qb_r0_2=0;
   ...
   AMRD_real_param(pfile,"phi4_qb_amp_1",&phi4_qb_amp_1,1);
   AMRD_real_param(pfile,"phi4_qb_x0_1",phi4_qb_x0_1,AMRD_dim);
   AMRD_real_param(pfile,"phi4_width_1",phi4_width_1,AMRD_dim);
   AMRD_real_param(pfile,"phi4_qb_delta_1",&phi4_qb_delta_1,1);
   AMRD_real_param(pfile,"phi4_qb_r0_1",&phi4_qb_r0_1,1);
   AMRD_real_param(pfile,"phi4_qb_w0_1",&phi4_qb_w0_1,1);

   AMRD_real_param(pfile,"phi4_qb_amp_2",&phi4_qb_amp_2,1);
   AMRD_real_param(pfile,"phi4_qb_x0_2",phi4_qb_x0_2,AMRD_dim);
   AMRD_real_param(pfile,"phi4_width_2",phi4_width_2,AMRD_dim);
   AMRD_real_param(pfile,"phi4_qb_delta_2",&phi4_qb_delta_2,1);
   AMRD_real_param(pfile,"phi4_qb_r0_2",&phi4_qb_r0_2,1);
   AMRD_real_param(pfile,"phi4_qb_w0_2",&phi4_qb_w0_2,1);
   ...
   use_phi1dat=0; AMRD_int_param(pfile,"use_phi1dat",&use_phi1dat,1);
   use_phi4dat=0; AMRD_int_param(pfile,"use_phi4dat",&use_phi4dat,1);
   ...
   if (ief_bh_r0 != 0.0 && ah_finder_is_off) 
   {
     real rh,rhoh,mh;
   ...
   if (use_phi1dat)
   {
      if (my_rank==0) printf("\nPhi001.csv initial data\n"
                             "xc=%lf, yc=%lf\n",phi1_x0_1[0],phi1_x0_1[1]);
   }

   if (use_phi4dat) 
   {
      if (my_rank==0) printf("\nphi4.dat initial data\n"
                             "xc=%lf, yc=%lf\n",phi4_qb_x0_1[0],phi4_qb_x0_1[1]);
   }

   if (AMRD_do_ex==0) AMRD_stop("require excision to be on","");

   PAMR_excision_on("chr",&AdS5D_fill_ex_mask,AMRD_ex,1);
...
//=============================================================================
void AdS5D_AMRH_var_clear(void)
{
   int i,j,ind;
...
// t0_cnst_data
//=============================================================================
void AdS5D_free_data(void)
{
   int i;

   ldptr();

   AdS5D_AMRH_var_clear(); // constrained variables are set post-MG

   zero_f(phi1_t_n); zero_f(phi4_r_t_n); zero_f(phi4_i_t_n); // holds initial time derivatives for ID
   ...
   if (use_phi1dat) phi1dat_(phi1_n,&phi1_x0_1[0],&phi1_x0_1[1],&AdS_L,x,y,&Nx,&Ny);

   if (use_phi1dat && (phi1_x0_1[0]!=0 || phi1_x0_1[1]!=0 || phi1_x0_2[0]!=0 || phi1_x0_2[1]!=0)) phi1dat_(w1,&phi1_x0_2[0],&phi1_x0_2[1],&AdS_L,x,y,&Nx,&Ny);

   for (i=0; i<size; i++) phi1_n[i]+=w1[i]; 

   if (use_phi4dat) phi4dat_(phi4_r_n,&phi4_qb_x0_1[0],&phi4_qb_x0_1[1],&AdS_L,x,y,&Nx,&Ny);

   approx_qb_(phi4_r_n,phi4_i_n,phi4_r_t_n,phi4_i_t_n,&phi4_qb_amp_1,&phi4_qb_r0_1,&phi4_qb_delta_1,
              &phi4_qb_x0_1[0],&phi4_qb_x0_1[1],&phi4_width_1[0],&phi4_width_1[1],&phi4_qb_w0_1,
              &use_phi4dat,&AdS_L,x,y,&Nx,&Ny);

   approx_qb_(w1,w2,w3,w4,&phi4_qb_amp_2,&phi4_qb_r0_2,&phi4_qb_delta_2,
              &phi4_qb_x0_2[0],&phi4_qb_x0_2[1],&phi4_width_2[0],&phi4_width_2[1],&phi4_qb_w0_2,
              &use_phi4dat,&AdS_L,x,y,&Nx,&Ny);

   for (i=0; i<size; i++) phi4_r_n[i]+=w1[i];
   for (i=0; i<size; i++) phi4_i_n[i]+=w2[i];
   for (i=0; i<size; i++) phi4_r_t_n[i]+=w3[i];
   for (i=0; i<size; i++) phi4_i_t_n[i]+=w4[i];
...
//=============================================================================
void AdS5D_elliptic_vars_t0_init(void)
{
...
void AdS5D_t0_cnst_data(void)
{
   ...
   if (skip_constraints && use_phi4dat!=0)
   {
     init_ghb_bs_(gb_tt,gb_tx,gb_ty,gb_xx,gb_xy,gb_yy,psi,&AdS_L,x,y,chr_mg,&AMRD_ex,&Nx,&Ny,&regtype);
   }
   ...
   if (AMRD_id_pl_method==3 && gb_xx_nm1)
   {
     ...
     init_nm1_(gb_tt_np1,gb_tt_n,gb_tt_nm1,gb_tt_t_n,
               ...
               phi4_r_np1,phi4_r_n,phi4_r_nm1,phi4_r_t_n,
               phi4_i_np1,phi4_i_n,phi4_i_nm1,phi4_i_t_n,
			   tfunction,
               &AdS_L,phys_bdy,x,y,&dt,chr,&AMRD_ex,&Nx,&Ny,&regtype);
...
void AdS5D_pre_io_calc(void)
{
   ...
   int j_shift;
   real ct,rho;
   real maxphisq_local[1],maxphisq_global[1];
   ...
   if (output_ires)
   {
      ...
      for (i=0; i<Nx; i++)
      {
         for (j=0; j<Ny; j++)
         {
            ...
            // fill in maxphisq 
			if (phi1_n[ind]*phi1_n[ind]>maxphisq_local[0]) {maxphisq_local[0]=phi1_n[ind]*phi1_n[ind];}
         } 
      }

   }

   // save max{maxphisq_local}_allprocessors into maxphisq_global
   MPI_Allreduce(maxphisq_local,maxphisq_global,1,MPI_DOUBLE,MPI_MAX,MPI_COMM_WORLD);

   // save max{phi1^2} to ascii_phi1
   if (my_rank==0)
   {
      FILE * fp;
      fp = fopen ("ascii_phi1", "a+");
          //format: t,max{phi1^2}
          fprintf(fp,"%24.16e %24.16e\n",ct,maxphisq_global[0]);
      fclose(fp);
   }
...
real AdS5D_evo_residual(void)
{
   real l2norm=0,l2norm_phi1,l2norm_phi4,l2norm_gb,l2norm_hb_t,l2norm_hb_i;
   int is_nan;
   ...
   if (LIN_ZERO_BND) 
   {
      ...
      lin_zero_bnd_res_(phi4_res,phys_bdy,&lin_zero_bnd_all,&Nx,&Ny);
   ...
   l2norm_phi4=norm_l2(phi4_res,mask,chr);
   ...
   if (!background) l2norm+=(l2norm_phi4+l2norm_gb+l2norm_hb_t+l2norm_hb_i);
...
//=============================================================================
//---lower frequency KO dissipation filter, called from AdS5D_evolve()
...
void apply_diss_eps_k(void)
{
   ...
   dmdiss3d_ex_gen_(phi4_r_n,w1,w2,&diss_bdy_k,pbt_even,&even,&odd,&Nx,&Ny,&Nz,w3,&AMRD_ex,&do_ex,&ind_sweeps,&diss_kmax);
   dmdiss3d_ex_gen_(phi4_i_n,w1,w2,&diss_bdy_k,pbt_even,&even,&odd,&Nx,&Ny,&Nz,w3,&AMRD_ex,&do_ex,&ind_sweeps,&diss_kmax);
   ...
   if (diss_all_past_k)
   {
      ...
      dmdiss3d_ex_gen_(phi4_r_nm1,w1,w2,&diss_bdy_k,pbt_even,&even,&odd,&Nx,&Ny,&Nz,w3,&AMRD_ex,&do_ex,&ind_sweeps,&diss_kmax);
      dmdiss3d_ex_gen_(phi4_i_nm1,w1,w2,&diss_bdy_k,pbt_even,&even,&odd,&Nx,&Ny,&Nz,w3,&AMRD_ex,&do_ex,&ind_sweeps,&diss_kmax);
...
void AdS5D_evolve(int iter)
{
   ...
   if (ltrace) printf("AdS5D_evolve: iter=%i , time=%lf, lev=%i, rank=%i\n",iter,ct,g_L,my_rank);
   ...
   g_evo_opt_(gb_res,phi1_res,phi4_res,cl_res,
              gb_tt_np1,gb_tt_n,gb_tt_nm1,
              gb_tx_np1,gb_tx_n,gb_tx_nm1,
              gb_ty_np1,gb_ty_n,gb_ty_nm1,
              gb_xx_np1,gb_xx_n,gb_xx_nm1,
...
//=============================================================================
void AdS5D_fill_ex_mask(real *mask, int dim, int *shape, real *bbox, real excised)
{
   int i,j,ind,l;
...
//=============================================================================
//=============================================================================
void AdS5D_fill_bh_bboxes(real *bbox, int *num, int max_num)
{
   if (max_num<MAX_BHS) AMRD_stop("AdS5D_fill_bh_bboxes: error max_num too small\n","");
...
void AdS5D_copy_block(char *p, char **q, int n, int dir, int *tot_size)
{
   char *p0;
   int n0;
   ...
   if ((*tot_size+n) > (CP_DATA_SIZE))
      AMRD_stop("AdS5D_copy_block: error ... CP_DATA_SIZE too small\n","");
   *tot_size+=n;
...
void AdS5D_cp(int dir, char *data)
{
   int size=0;
   ...
   {
       cp_version=ADS5D_CP_VERSION;
       AdS5D_copy_block((char *)&cp_version,&data,sizeof(int),dir,&size);
   }
}
...
{
   amrd_set_app_user_cp_hook(AdS5D_cp,CP_DATA_SIZE);
   amrd_set_app_pre_tstep_hook(AdS5D_pre_tstep);
   amrd_set_elliptic_vars_t0_init(AdS5D_elliptic_vars_t0_init);
   amrd(argc,argv,&AdS5D_id,&AdS5D_var_pre_init,
        &AdS5D_var_post_init, &AdS5D_AMRH_var_clear,
        &AdS5D_free_data, &AdS5D_t0_cnst_data,
        &AdS5D_evo_residual, &AdS5D_MG_residual,
        &AdS5D_evolve, &AdS5D_MG_relax, &AdS5D_L_op, 
        &AdS5D_pre_io_calc, &AdS5D_scale_tre, 
        &AdS5D_post_regrid, &AdS5D_post_tstep,
        &AdS5D_fill_ex_mask, &AdS5D_fill_bh_bboxes);
}
